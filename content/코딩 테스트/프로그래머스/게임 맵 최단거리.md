...
{
    int y, x, cnt;
    Position(int y, int x, int cnt) : y(y), x(x), cnt(cnt) {}
};
...
int bfs(vector<vector<int>> &maps)
{
    ...
    nextPos.push(Position(0, 0, 1));
    
    while(!nextPos.empty())
    {
        Position currentPos = nextPos.front();
        nextPos.pop();
        
        if(currentPos.y == n-1 && currentPos.x == m-1)
        {
            return currentPos.cnt;
        }
        
        for(int i= 0; i < 4; i++)
        {
            int nextY = currentPos.y + ny[i];
            int nextX = currentPos.x + nx[i];
            if(isInMap(nextY, nextX) && !visited[nextY][nextX] && maps[nextY][nextX] != 0)
            {
                nextPos.push(Position(nextY, nextX, currentPos.cnt + 1));
