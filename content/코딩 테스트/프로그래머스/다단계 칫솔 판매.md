...
void ToParent(vector<int> &answer, vector<string> enroll, vector<string> referral, int amount, int childIndex);
...
vector<int> solution(vector<string> enroll, vector<string> referral, vector<string> seller, vector<int> amount) {
    ...
    // > seller < length 길이만큼 반복
    // 1. seller의 이름을 enroll에서 찾음
    // 2. enroll에서 찾은 노드의 부모를 referral에서 찾음
    // 3. enroll에서 돈 10% 까고 깐 돈을 referral한테 보냄
    // 4. 그걸 referral의 부모로 또 반복
    
    for(int i = 0; i < seller.size(); i++)
    {
        auto it = find(enroll.begin(), enroll.end(), seller[i]);
        int childIndex = distance(enroll.begin(), it);
        ToParent(answer, enroll, referral, amount[i], childIndex);
    }
...
void ToParent(vector<int> &answer, vector<string> enroll, vector<string> referral, int amount, int childIndex)
{
    ...
    float floatParentMoney = amount * 0.1f;
    if(parentMoney < 1)
    {
        parentMoney = 0;
        childMoney = amount;
    ...
    {
        parentMoney = (int)(amount * 0.1f);
        childMoney = amount - parentMoney;
    }
    
    // 일단 내 돈 챙기기
    answer[childIndex] += childMoney;
    
    // cout << "Child : " << childIndex;
    
    if(parentMoney == 0) return;
    
    auto it = find(enroll.begin(), enroll.end(), referral[childIndex]);
    if (it == enroll.end()) // 부모가 없는 경우 : 그만 돌림
    {
        // cout << " No Parent" << endl;
        return;
    }
    ...
    int parentIndex = distance(enroll.begin(), it);    
    
    // cout << "  Parent : " << parentIndex << endl;
    
    // 부모있으면 부모한테 10퍼 넘기기
    ToParent(answer, enroll, referral, parentMoney, parentIndex);
}

```

- 시간 부족 뜸
- 왜일까 생각해 봤는데, 아무래도 find 때매 그런듯
- 우수 코드에서 봤을 때 unordered map을 쓰더라..
...
void ToParent(unordered_map<string, string> &parents, unordered_map<string, int> &money, int amount, string childName);
...
vector<int> solution(vector<string> enroll, vector<string> referral, vector<string> seller, vector<int> amount) {
    ...
    // 나중에 100 곱하기 귀찮아서 지금 함
    for(int i = 0; i < amount.size(); i++)
    {
        amount[i] *= 100;
    }
    
    // key가 가지고 있는 돈은 value임
    ...
    for(int i = 0; i < seller.size(); i++)
    {
        ToParent(parents, money, amount[i], seller[i]);
    }
...
void ToParent(unordered_map<string, string> &parents, unordered_map<string, int> &money, int amount, string childName)
{
    ...
    ToParent(parents, money, parentMoney, parents[childName]);
}
...
- 주요 로직 자체는 내가 짠거랑 같음
- 차이는 자료구조에 있음
- 부모와 현재 가지고있는 돈을 unordered_map으로 관리해서 부모 찾거나 할 때 O(1) 속도가 나옴