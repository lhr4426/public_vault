...
class BinaryTree
{
    ...
    Node* addNode(Node *currentNode, Node *newNode)
    {
    ...
    void preOrder(Node *node, vector<int> &trace)
    {
        if (node == nullptr)
            return;
        trace.push_back(node->id);
        preOrder(node->left, trace);
        preOrder(node->right, trace);
    }
    
    void postOrder(Node *node, vector<int> &trace)
    {
        if (node == nullptr)
            return;
        postOrder(node->left, trace);
        postOrder(node->right, trace);
        trace.push_back(node->id);
    }
    ...
    void buildTree(vector<vector<int>> &nodeInfo)
    {
        ...
        {
            nodes.push_back(new Node(i, nodeInfo[i][0], nodeInfo[i][1]));
        }
...
vector<vector<int>> solution(vector<vector<int>> nodeinfo) {
    ...
    answer.push_back(tree.getPreOrder());
    answer.push_back(tree.getPostOrder());
    return answer;
}
```
...
Node* addNode(Node *currentNode, Node *newNode)
    {
        ...
        if (compareNodes(currentNode, newNode))
        {
            addNode(currentNode->left, newNode); // 새 노드 연결을 안해줌줌
        }
        else
        {
            addNode(currentNode->right, newNode);
