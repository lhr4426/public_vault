...
int solution(int n, vector<int> weak, vector<int> dist) {
    // expand weak
    int oneCycleLength = weak.size(); // 일부러 확장함
    ...
    for(int start = 0; start < oneCycleLength; start++)
    {
        do
        {
            int cnt = 1;
            ...
            for(int k = start; k < start + oneCycleLength; k++)
            {
                if(position < weak[k])
                {
                    cnt++;
                    if(cnt > dist.size()) break;
                    position = weak[k] + dist[cnt - 1];
                }
            }
